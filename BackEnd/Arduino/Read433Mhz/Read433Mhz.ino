...
static void startReceiving(int _interruptPin) {
  footer_length = 0;
  state = STATUS_WAITING;
  data_end[0] = 0;
  data_end[1] = 0;
...
static void getRaw(unsigned int **buffer, unsigned int* timings_size) {
  if (data1_ready){
    *buffer = &timings[0];
...
static void sendByCompressedTimings(int transmitterPin,unsigned long* buckets, char* compressTimings, unsigned int repeats) {
  listenBeforeTalk();
...
static void sendByTimings(int transmitterPin, unsigned int *timings, unsigned int timings_size, unsigned int repeats) {
  listenBeforeTalk();
